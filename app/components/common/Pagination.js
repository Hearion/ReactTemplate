...
export default class Pagination extends Component{
    ...
    componentWillMount(){
        ...
        if(this.props.serialNo && columns[0].dataIndex!==serialNo){
            if(this.props.property){
                columns.unshift({title: "序号",dataIndex: serialNo,className:"tableAlignCenter",width:80,render:(text,record,index)=><span style={{color:record[this.props.property]?"#ff3333":"inherit"}}>{record.serialNo}</span>});
            }else{
                columns.unshift({title: "序号",dataIndex: serialNo,className:"tableAlignCenter",width:80});
            }
        }
    ...
    componentWillReceiveProps(props){
        ...
        if( props.serialNo && columns[0].dataIndex!==serialNo){
            if(props.property){
                columns.unshift({title: "序号",dataIndex: serialNo,className:"tableAlignCenter",width:80,render:(text,record,index)=><span style={{color:record[props.property]?"#ff3333":"inherit"}}>{record.serialNo}</span>});
            }else{
                columns.unshift({title: "序号",dataIndex: serialNo,className:"tableAlignCenter",width:80});
            }
        }
    ...
    render(){
        ...
        return(
            ...
            <Table size="middle" bordered={bordered} expandedRowRender={expandedRowRender} rowClassName={this.rowClassName} rowKey={rowKey} rowSelection={rowSelect?rowSelection:null} 
                ...
                pagination={pagination?{total:dataProvider.totalCount,pageSize:dataProvider.pageSize,current:dataProvider?dataProvider.pageNo:null,showTotal:total => `共 ${total} 条`,showSizeChanger:true,pageSizeOptions:['10','30','50','70','100','1000']}:false} scroll={scroll}/>
                :null
        )
    }
}
